...
class MyGoogleAuthenticator extends OAuth2Authenticator implements AuthenticationEntrypointInterface
{
    ...
    public function authenticate(Request $request): Passport
    {
        ...
        $roles = ["ROLE_CLI"];
        $client = $this->clientRegistry->getClient('google_cliente');

        if ($entorno === 'proveedor') {
            $roles = ["ROLE_PROV"];
            $client = $this->clientRegistry->getClient('google_proveedor');
        }
        ...
        return new SelfValidatingPassport(
            new UserBadge($accessToken->getToken(), function () use ($accessToken, $client, $roles) {
                ...
                if ($user) {
                    ...
                    $user = new Usuario();
                    $person = new Persona();
                    $person->setPNombre($googleUser->getFirstName());
                    $person->setPApellido($googleUser->getLastName());
