...
//        ;
//    }
    public function findMensajesByConversacionIdAndUserId(int $conversacionId, int $userId)
    {
        $qb = $this->createQueryBuilder('p');
        $qb->
        where(
            $qb->expr()->andX(
                $qb->expr()->eq('p.conversacion_id', ':conversacionId'),
                $qb->expr()->neq('p.usuario_id', ':userId')
            )
        )
        ->setParameters([
